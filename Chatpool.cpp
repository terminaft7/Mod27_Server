...
bool chatpool::showmap(string receiver, string nickname)
{
    ...
    //  this->logread(nickname, receiver);
...
void chatpool::processRequest(string receiver, string nickname)
{
    ...
    if (strcmp(receiver_char, buffer) == 0)
    {
        ...
        while (1)
        {
            ...
            Logger logger1;
            std::thread logThread1(&Logger::logmessage, &logger1, nickname, receiver, buffer);
            logThread1.join();
            ...
            std::thread logThread2(&Logger::logmessage, &logger2, nickname, receiver, send_buffer);
            logThread2.join();
    ...
    {
        cout << "Sorry, the user you want to talk to is not online. Please try later." << endl;
        char answer[2] = "0";
