    ...
    while (action != 'q')
    {
        ...
        case '1':
            ...
            if (ind == true)
            {
                ...
                /// Запускаем функцию чтения сообщений из базы данных
                ...
                if (ind_showmap == true)
                {
                    ...
                    // Запускаем функцию чтения сообщений из лога в отдельном потоке. Сообщения будут выведены вслед за сообщениями из базы данных.
                    // Удобно сравнить и проверить, что всё работает.
                    Logger reader;
                    std::thread logreadThread1(&Logger::logread, &reader, nickname, receiver);
                    logreadThread1.join();
